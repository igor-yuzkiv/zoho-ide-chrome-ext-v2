import { WorkflowSchemaBuilderRegister } from '@/modules/capabilities/workflows/workflows.config.ts'
...
export function useWorkflowSchema(): {
    ...
    function render(providerType: ZohoServiceProviderType, workflow: IWorkflowRecordEntity) {
        ...
        if (!schemaBuilder || typeof schemaBuilder !== 'function') {
            console.warn(`No workflow schema builder found for provider type: ${providerType}`)
            return
        }
        ...
        if (!result.ok) {
            clear()
            console.warn(`Failed to build workflow schema for provider type: ${providerType}`, result.error)
            return
        }
