...
import type { CrmModuleMetadata } from '@/shared/integrations/zoho-crm/types/crm.metadata.types.ts'
...
async function regular(tabId: number, orgId: string): Promise<Result<CrmModuleMetadata[]>> {
    ...
    const response = await zohoCrmRequest<{ modules: CrmModuleMetadata[] }>(tabId, {
